...
=Scripts/increment_build_number.sh= increments every =CURRENT_PROJECT_VERSION=
in the Xcode project together. Builds, runs, and tests do not change the build
number. Xcode Cloud assigns TestFlight build numbers on =main= independently of
the project file; local archives continue to increment the project version
normally.
...
The workflow lives in App Store Connect (Xcode → Product → Xcode Cloud, or App
Store Connect → Xcode Cloud), not as YAML in this repository. Set
