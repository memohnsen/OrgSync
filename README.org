...
Pushes (including merged pull requests) to =main= run =mise run publish= in GitHub
Actions. Before enabling the workflow, add these repository Action secrets:
...
- =BUILD_PROVISION_PROFILE_BASE64=: the App Store provisioning profile,
  base64-encoded.
