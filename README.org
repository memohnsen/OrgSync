...
- [[https://mise.jdx.dev/][Mise]] is optional but recommended for the development
  commands below.
...
=mise run run= uses the configured simulator ID. Override it when needed:

#+begin_src sh
IOS_SIMULATOR_ID="YOUR-SIMULATOR-UUID" mise run run
#+end_src
...
build number. The TestFlight workflow supplies a unique =CI_BUILD_NUMBER=, so a
fresh GitHub Actions checkout does not reuse a build number; local archives
continue to increment the project version normally.

* TestFlight deployment

Pull requests build an unsigned Release archive in parallel with unit tests, so
the complete app and widget archive graph is verified without exposing signing
credentials. The archive is rebuilt with distribution signing for deployment.

After unit tests pass, pushes (including merged pull requests) to =main= run
=mise run publish= in GitHub Actions and distribute the processed build to the
configured TestFlight group. Before enabling the workflow, add these repository
Action secrets:

- =ASC_KEY_ID=, =ASC_ISSUER_ID=, and =ASC_PRIVATE_KEY_B64=: the App Store
  Connect API key used by =asc=. Encode the =.p8= key with
  =base64 -i AuthKey_XXXXXXXXXX.p8 | pbcopy=.
- =BUILD_CERTIFICATE_BASE64= and =P12_PASSWORD=: the distribution certificate
  exported as a password-protected =.p12= and base64-encoded.
- =BUILD_PROVISION_PROFILE_BASE64= and =WIDGET_PROVISION_PROFILE_BASE64=: the
  App Store provisioning profiles for the app and widget extension,
  respectively, base64-encoded.
- =KEYCHAIN_PASSWORD=: an arbitrary strong, repository-secret password used
  only for the temporary CI keychain.
- =REVENUECAT_API_KEY_LIVE=: the public RevenueCat production SDK key injected
  into release archives.

Also set the =ASC_TESTFLIGHT_GROUP= repository Actions variable to the name or
ID of the internal or external TestFlight group that should receive each build.
