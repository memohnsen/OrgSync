...
final class RemindersSyncEngine {
    ...
    func sync(repo: RepoStore) async {
        ...
        do {
            ...
            let phases = ReminderSyncRules.syncPhases(for: settings.remindersMasterSource)
            ensurePersistentIDs(repo: repo)

            var byKey = prepareTodoLookup(repo: repo, mappings: &mappings)

            if phases.contains(.importFromReminders) {
                byKey = await importFromReminders(
                    list: list, mappings: &mappings, byKey: byKey, repo: repo
                )
            }
    ...
    private func prepareTodoLookup(
        repo: RepoStore, mappings: inout [String: String]
    ) -> [String: OrgTodoItem] {
    ...
    private func importFromReminders(
        ...
        byKey: [String: OrgTodoItem],
        repo: RepoStore
    ) async -> [String: OrgTodoItem] {
        var byKey = byKey
